...
import { fetchAllUsers } from "@/services/systemUser.service";
import { ProgressIndicator } from "@/components/progressIndicator/progressIndicator";
import { accessToRows, defaultFilterConfig, headOfColumns, generateDynamicInputs, bottonActionConfig } from "@/config/userOfSystemPage.config";
import { confirmAccessRoute } from "@/util/confirmRouteAccess";

const UserOfSystemPage = () => {
    const { user } = useUser();

    if (user && confirmAccessRoute(user.role)) {
        const { rows: initialRows, isLoaded } = useFetchData(fetchAllUsers);
...
export default UserOfSystemPage;